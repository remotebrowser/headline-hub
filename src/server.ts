...
app.get('/api/news', async (req, res) => {
  ...
  try {
    ...
    conn = await createRemoteBrowser(browserId, headers);

    page = await createPage(conn, browserId);
    ...
    if (page && conn) {
      try {
        await closeTarget(conn, page.targetId);
      } catch (e) {
        Logger.error('Error closing target:', e as Error);
      }
    }
    if (conn) {
      cdpDisconnect(conn);
